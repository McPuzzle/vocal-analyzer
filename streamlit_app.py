...
def detect_surgical_res(freqs, dry_db, ref_db):
    diff = dry_db - ref_db
    idxs,props = find_peaks(dry_db, height=np.max(dry_db)*0.05)
    res = []
    for i in idxs:
...
def generate_chain(broad, res, s_dry, s_ref, bpm):
    chain=[]
    chain.append({'step':'Cleaning','plugin':'Pro-Q3','settings':[
        f"HPF {guidelines['highpass_cutoff']}Hz {guidelines['highpass_slope']}dB/oct"]})
    if broad:
        s=[f"{c['band']} EQ @ {c['center']:.0f}Hz Q{c['q']} Gain{c['gain']:+.1f}dB" for c in broad]
        chain.append({'step':'Broad EQ','plugin':'Pro-Q3','settings':s})
    if res:
        s=[f"Notch @ {r['freq']:.0f}Hz Q{r['q']:.1f} Cut-{r['cut']:.1f}dB" for r in res]
        chain.append({'step':'Surgical Notches','plugin':'Pro-Q3','settings':s})
        ...
        s2=[f"Dynamic @ {r['freq']:.0f}Hz Q{r['q']:.1f} Th-{r['cut']*0.75:.1f}dB "+
            f"Atk{atk_nm}({atk_ms}ms) Rel{rel_nm}({rel_ms}ms)" for r in res]
        chain.append({'step':'Dynamic EQ','plugin':'Pro-Q3 Dyn','settings':s2})
    if s_dry > s_ref + guidelines['sibilance_threshold_rms']:
        ...
        chain.append({'step':'De-essing','plugin':'Pro-DS','settings':[
    ...
    s3=[f"{p}:R{r}:1 Atk{atk_nm} Rel{rel_nm}" for r,p in guidelines['compression_ratios']]
    chain.append({'step':'Compression','plugin':'Serial','settings':s3})
    chain.append({'step':'Saturation','plugin':'Parallel','settings':[
        f"SatBlend{int(guidelines['saturation_blend']*100)}%",
        "+0.5dB HS@10kHz"]})
    return chain
...
with col1: dry=st.file_uploader("Dry WAV",type='wav')
with col2: ref=st.file_uploader("Ref WAV",type='wav')

if ref:
    tmp=tempfile.NamedTemporaryFile(suffix='.wav',delete=False)
    tmp.write(ref.read()); tmp.close()
    key,bpm=detect_key_bpm(tmp.name); os.unlink(tmp.name)
    st.write("Key:",key,"BPM:",bpm)

if dry and ref and st.button("Analyze"):
    # Save temps
    dt=tmpfile1=tempfile.NamedTemporaryFile(suffix='.wav',delete=False)
    rt=tmpfile2=tempfile.NamedTemporaryFile(suffix='.wav',delete=False)
    dt.write(dry.read()); dt.close()
    rt.write(ref.read()); rt.close()
    ...
    os.unlink(dt.name); os.unlink(rt.name)
