...
if dry_file and ref_file and st.button("Analyze Vocals"):
    ...
    for band, (fmin, fmax) in bands.items():
        ...
        for i in top:
            ...
            calculated_Q = cf / bw if bw > 0 else 10
            Q = max(5, min(calculated_Q, 15))  # Clamp Q between 5 and 15
    ...
    for i, row in df_ed.iterrows():
        ...
        q = st.slider("Q", 1.0, 15.0, value=float(row["Q"]), step=0.1, key=f"q{i}")  # Limit Q max at 15
...
if st.session_state.chain_ready:
    ...
    st.write(f"• Final manual gain rides and A/B vs reference (Key: {st.session_state.ref_key}, BPM: {st.session_state.ref_bpm})")
