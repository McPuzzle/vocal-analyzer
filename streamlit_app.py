...
# 3) Key/BPM detection and override
...
if dry_file and ref_file and st.button("Analyze Vocals"):
    ...
    for band,(fmin,fmax) in bands.items():
        idx=(freqs>=fmin)&(freqs<=fmax); bm, bf = mag_dry[idx], freqs[idx]
...
if st.session_state.chain_ready:
    ...
    if st.button("Generate Engineer-Ready Chain"):
        ...
        st.write(f"• De-Esser 5–8 kHz — Threshold {th_min}–{th_max} dB, Ratio {r_min}:{r_max}")
        st.write(f"• A/B vs reference (Key: {st.session_state.ref_key}, BPM: {st.session_state.ref_bpm})")
