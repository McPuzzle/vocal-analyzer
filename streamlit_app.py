...
# 2) Pre-Analysis Key/BPM UI (always rendered)
st.subheader("🔑 Key & 🎵 Tempo Detection (Auto + Manual Override)")
...
# Detect and store auto values in session_state once files are uploaded
if dry_file and 'auto_dry_key' not in st.session_state:
    with tempfile.NamedTemporaryFile(suffix='.wav', delete=False) as t:
        t.write(dry_file.read())
        st.session_state.auto_dry_key, st.session_state.auto_dry_bpm = detect(t.name)
        os.unlink(t.name)

if ref_file and 'auto_ref_key' not in st.session_state:
    with tempfile.NamedTemporaryFile(suffix='.wav', delete=False) as t:
        t.write(ref_file.read())
        st.session_state.auto_ref_key, st.session_state.auto_ref_bpm = detect(t.name)
        os.unlink(t.name)
...
# Dry vocal auto-display & override
...
# Reference vocal auto-display & override
...
if dry_file and ref_file and st.button("Analyze Vocals"):
    # Re-save temp for spectrum and peaks
    ...
    if st.button("🔧 Build Iterative Mix Chain"):
        ...
        st.write(f"• Comp A 4:1 — Attack~{atk_sub}({atk_ms}ms), Release~{rel_sub}({rel_ms}ms), GR~2dB")
        st.write(f"• Comp B 2.5:1 — Attack~{atk_sub}({atk_ms}ms), Release~{rel_sub}({rel_ms}ms), GR~2dB")

        st.subheader("Phase 3: Additive Passes")
        st.write("**Pass E (Shelving & Parallel Sat 5–10%)**")
        for _, r in s.iterrows():
            st.write(f"• Parallel Notch@{r['Center']:.1f}Hz—Q0.7,Cut{r['P5']}dB")
        st.write("• High-Shelf@10kHz—+1.5dB,Q0.7")
        st.write("• Parallel Sat Bus(HP800Hz,LP12kHz)@10%blend")
        ...
        st.write(f"• De-Esser5–8kHz—Thresh{th_min}–{th_max}dB,Ratio{r_min}:{r_max}")
